...
def process_sheet(sheet):
    ...
    # check sheet name
    sheet_match = re_sheet_name.fullmatch(sheet)
    if not sheet_match:
    ...
    rows = list(csv.reader(open('sheets/'+sheet,'r')))
    ...
    for i in range(2,len(rows)):
        ...
        for region in songlist_col:
            ...
            if os.path.exists('ranked_data/'+filename):
                data = open('ranked_data/'+filename,'r').read()
                if json_valid(data):
                    redump = json.dumps(json.loads(data),indent=4)
                    if redump != data: # rewrite file with json reformatted
                        file = open('ranked_data/'+filename,'w')
                        file.write(redump)
                        file.close()
