// CPU version of the subset generator
// subset sum approximation optimized with the 12sum version from cachetest.c
// LCG iteration optimized with SSE2/SSE4.1 instructions (Intel Nehalem+)
// parallelized with pthread library, global mutex for workunit distribution

// the numbers higher bits are hardcaded (discarding lower 67 bits)
// approximated subset sum is off by ~3.7*10^21 on average, small compared to
// the 2^79 ~= 6*10^23 range used

// the range chosen begins at 435939*2^79 ~= 2.6*10^29 which was the average
// subset sum of subsets generated by using a full period LCG for testing
// this range is at approximately half the sum of all 100 numbers
...
// cacheX[i] = sum(HI27[base+b] if bit b is set)
// i represents whether to include/exclude some of the numbers in a subset
// base is the lowest number index for the cache
void cache_generate()
{
    // i = cache index
...
// t_num = small number to use instead of pthread_self()
// fd = file descriptor for the thread's output file
...
uint32_t *get_work_unit()
{
    ...
    ssize_t bytes = read(wu_fd,buf,96); // critical section
    if (pthread_mutex_unlock(&wu_lock))
    {
        printf("ERROR unlocking mutex\n");
    }
    if (bytes < 96) // EOF/error or file not a multiple of 96 bytes
    {
        if (bytes > 0)
        {
...
// 4MiB is enough to have each work unit flush once completed
// most work units generate 1.0MB or 1.1MB of data (~65000 subsets)
#define TFILE_BUF (1<<22)

// SIMD version, using SSE2 and SSE4.1, only a little faster
// the LCG iteration is much faster
// the L1 cache accesses for subset summing are much slower
// lcgs must contain 24 uint32_t values (96 bytes)
// fd is the file descriptor to write output to
// t_num is thread number
// buf is the array to use for a manually managed file buffer
// satisfying subsets are written to the provided file descriptor
// the work units run 2 independent subset generating sequences in order to
// saturate the CPU pipeline further for improved performance
void *subset_generator_sse(void *lcgs, int fd, uint32_t t_num, uint8_t *buf)
{
    ...
    // load parameters from the 96 byte work unit
    // generates ~8.5 billion subsets
    ...
    uint32_t set0[4], set1[4]; // store generated subsets
    ...
    uint32_t sscount = 0; // count of subsets found
    while (counter--) // iterate 2^32-1 numbers from each LCG
    {
        // advance state of the 4 LCGs, X0 and X1 store new subsets
        ...
        // slow part, access caches to sum with fewer operations
        // first 64 bits are taken from first 8 bytes
        // last 36 bits are taken as the high/low 9 bits from last 2 uint32_t
        bytes = (uint8_t*) set0;
        ...
        if (sum0 >> 12 == 435939) // hardcoded search range
        {
            ++sscount;
            memcpy(buf_ptr,set0,16);
            buf_ptr += 16;
            if (buf_ptr == buf_end) // flush buffer
            {
        ...
        if (sum1 >> 12 == 435939) // hardcoded search range
        {
            ++sscount;
    ...
    if (write(fd,buf,buf_ptr-buf) != buf_ptr-buf) // flush buffer
        printf("[thread %u] ERROR writing subsets\n",t_num);
...
void *worker_function(void *params)
{
    ...
    uint32_t completed = 0; // count work units completed
    uint32_t *retval; // return value from a work unit
    uint64_t found = 0; // total subsets found
    uint8_t *file_buf = malloc(TFILE_BUF); // manually managed file buffer
    int fd = params_cast->fd;
    clock_t beg,end; // measure time
...
int main(int argc, char **argv)
{
    ...
    char filepath[strlen(argv[3])+32]; // output path buffer
    for (uint32_t t = 0; t < thread_count; ++t) // create threads
    {
    ...
    for (uint32_t t = 0; t < thread_count; ++t) // join threads
    {
